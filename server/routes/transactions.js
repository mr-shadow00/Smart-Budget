...
// If this entry was an auto-generated savings deposit/withdrawal, deleting
// it also reverses its effect on that goal's saved amount — so the goal
// and the ledger can never drift out of sync with each other.
router.delete('/:id', async (req, res) => {
  ...
  await withLock((data) => {
    ...
    if (tx.source === 'savings' && tx.goalId && Array.isArray(data.savings)) {
      const goal = data.savings.find((g) => g.id === tx.goalId);
      if (goal) {
        if (tx.type === 'expense') {
          // This entry was a deposit into the goal — reverse it.
          goal.currentAmount = Math.max(0, Math.round(((goal.currentAmount || 0) - tx.amount) * 100) / 100);
        } else if (tx.type === 'income') {
          // This entry was a withdrawal from the goal — reverse it.
          goal.currentAmount = Math.round(((goal.currentAmount || 0) + tx.amount) * 100) / 100;
        }
      }
    }
