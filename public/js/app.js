    ...
    items.forEach((t) => {
      ...
      row.innerHTML = `
        ...
        <div class="tx-main">
          ...
          <div class="tx-cat">${t.category ? `<span>${escapeHtml(t.category)}</span>` : ''}</div>
        </div>
  ...
  function renderTxList(transactions) {
    ...
    groups.forEach((g) => {
      ...
      g.items.forEach((t) => {
        ...
        const row = document.createElement('div');
        row.className = 'tx-row' + (hasNote ? ' has-note' : '');
        ...
        row.innerHTML = `
          ...
          <div class="tx-main">
            <div class="tx-desc">${escapeHtml(t.description || t.category || 'Untitled')}</div>
            <div class="tx-cat">${t.category ? `<span>${escapeHtml(t.category)}</span>` : ''}${hasNote ? '<span class="tx-note-dot">🗒️ Note</span>' : ''}</div>
          </div>
