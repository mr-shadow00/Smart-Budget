...
(() => {
  ...
  async function loadCharts() {
    const month = state.allTime ? '' : monthKey(state.month.year, state.month.month);
    $('#charts-month-label').textContent = state.allTime ? 'All time' : monthLabel(state.month.year, state.month.month);
    ...
    renderCategoryDonut(summary.byCategory);
    renderExpenseBreakdownList(summary.byCategory);
  }
  ...
  function categoryEntries(byCategory) {
  ...
  function renderCategoryDonut(byCategory) {
    const allEntries = categoryEntries(byCategory);
    const wrapEl = $('#category-chart');
    const legendEl = $('#category-legend');
    const emptyEl = $('#category-chart-empty');
    ...
    const svg = `
      <svg viewBox="0 0 ${size} ${size}" width="${size}" height="${size}">
        ...
        <text class="donut-center-sub" x="${cx}" y="${cy + 16}" text-anchor="middle">Spent</text>
      </svg>
    `;
    ...
    entries.forEach((e) => {
      ...
      row.innerHTML = `
        <div class="ring-legend-top">
          <span class="ring-legend-name"><span class="ring-legend-dot" style="background:${color}"></span><span>${escapeHtml(e.name)}</span></span>
          <span class="ring-legend-figures">
            <span class="ring-legend-value">${fmtMoney(e.amount, state.settings.currency)}</span>
            <span class="ring-legend-pct">${pct}%</span>
          </span>
        </div>
        <div class="ring-legend-track"><div class="ring-legend-fill" style="width:${pct}%;background:${color}"></div></div>
      `;
      legendEl.appendChild(row);
    });
  }

  function renderExpenseBreakdownList(byCategory) {
    const entries = categoryEntries(byCategory);
    const listEl = $('#expense-breakdown-list');
    const emptyEl = $('#expense-breakdown-empty');
    listEl.innerHTML = '';
